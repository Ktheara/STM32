...
// Any size beyond will require you to change EEPROM_MEM_ADDR_SIZE to 2 or further. The header has not been changed.
#define EEPROM_MEM_ADDR_SIZE 1

#define BYTES_TO_GO(size, offset) size+offset<PAGE_SIZE?size:PAGE_SIZE-offset;

const int PAGE_SIZE_EXPONENT = log(PAGE_SIZE)/log(2);
...
void EEPROM_Write (uint16_t page, uint16_t offset, uint8_t *data, uint16_t size)
{
	...
	for (; startPage <= endPage; startPage++)
	{
		...
		uint16_t bytestowrite = BYTES_TO_GO(size, offset);  // calculate the remaining bytes to be written

		HAL_I2C_Mem_Write(EEPROM_I2C, EEPROM_ADDR, MemAddress, EEPROM_MEM_ADDR_SIZE, &data[bytepos], bytestowrite, 1000);  // write the data to the EEPROM

		// startPage += 1;  // increment the page, so that a new page address can be selected for further write
		offset = 0;   // since we will be writing to a new page, so offset will be 0
		size -= bytestowrite;  // reduce the size of the bytes
		bytepos += bytestowrite;  // update the position for the data buffer
...
{
	//int PAGE_SIZE_EXPONENT = log(PAGE_SIZE)/log(2);
	...
	for (; startPage <= endPage; startPage++)
	{
		...
		HAL_I2C_Mem_Read(EEPROM_I2C, EEPROM_ADDR, MemAddress, EEPROM_MEM_ADDR_SIZE, &data[bytepos], bytestoread, 1000);
		//startPage += 1;
		offset = 0;
		size -= bytestoread;
		bytepos += bytestoread;
